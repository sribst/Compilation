...
(* let bind_all_function env p = *)
(*   let f_fold env = function *)
(*     | S.DefFun (fid, _, _) *)
(*     | S.DefCont (fid,_,_,_) -> *)
(*       let flbl,env = bind_function_label env fid in *)
...
and basicexpr fun_box env = function
  ...
  | S.FunName f ->
    let addr = Labels.encode (lookup_function_label f env) in
...
and continuation env = function
  ...
  | S.PushCont(kont, cont_and_env, m) ->
    ...
    let kont_addr = Labels.encode (lookup_function_label kont env )
    in
