...
and expression is_tail fun_box env = function
  ...
  (* Entier : on le pousse sur la pile *)
  | S.Num i -> fun_box unboxed (push i)

  (* Variable : on recupere sa valeur dans l'environment,
     on la place danse la pile *)
  ...
  (* Fonctions :
     - on récupere le label et les arguments de l'identificateur de
       fonction *)
  | S.FunName f ->
    let fl = lookup_function_label f env in
